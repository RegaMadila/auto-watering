...
FuzzySet *normal = new FuzzySet(15, 25, 30, 35);
FuzzySet *hot = new FuzzySet(35, 45, 50, 50 );
...
FuzzySet *mediumDuration = new FuzzySet(0, 2, 4, 5);
...
volatile bool manualWateringActive = false;
volatile unsigned long manualWateringStartTime = 0;
volatile unsigned int manualWateringDuration = 0;

void activateManualWatering(bool turnOn) {
  if (turnOn) {
    digitalWrite(RELAY_PIN, LOW);  // Nyalakan relay
    manualWateringStartTime = millis();  // Catat waktu mulai penyiraman manual
    manualWateringActive = true;  // Set status penyiraman manual aktif
  } else {
    digitalWrite(RELAY_PIN, HIGH);  // Matikan relay
    manualWateringStartTime = 0;  // Reset waktu mulai penyiraman manual
    manualWateringActive = false;  // Set status penyiraman manual tidak aktif
  }
}
...
void sendSensor()
{
  ...
  float soilHumidityInput = map(analogRead(SOIL_MOISTURE_PIN), 0, 4095, 100, 0);
  // float soil =  ( 100 - ( (analogRead(soil_moisture_pin) / 4095.00) * 100 ) );
    
  if (isnan(humidityInput) || isnan(temperatureInput) || soilHumidityInput < 0) {
  ...
  if (manualWateringActive) {
    ...
    unsigned long elapsedMillis = currentTime - manualWateringStartTime;

    if (elapsedMillis >= (manualWateringDuration * 1000)) {
      digitalWrite(RELAY_PIN, HIGH);  // Matikan relay setelah durasi tercapai
      manualWateringStartTime = 0;  // Reset waktu mulai penyiraman manual
      manualWateringActive = false;  // Set status penyiraman manual tidak aktif
    }
  }
...
void setup()
{
  ...
  timer.setInterval(10000L, sendSensor);  // Adjust the interval as needed
}

BLYNK_WRITE(V5) {
  manualWateringActive = param.asInt() == 1;  // Mengaktifkan atau menonaktifkan manual watering
  if (manualWateringActive) {
    manualWateringStartTime = millis();  // Catat waktu mulai penyiraman manual
  }
}
